from typing import List, Any
from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from sqlalchemy.exc import IntegrityError

from app.api.deps import get_current_staff , get_current_user
from app.crud.crud_doctor import doctor
from app.schemas.doctor import Doctor, DoctorCreate, DoctorUpdate,DoctorWithAvailability, AvailabilityCreate, AvailabilityUpdate
from app.schemas.user import User
from app.db.session import get_db

router = APIRouter()

@router.get("/",response_model=List[Doctor], status_code=status.HTTP_200_OK)
def read_doctors( 
    skip : int = 0 , limit : int = 100,
    db: Session = Depends(get_db),
) -> Any:
    doctors = doctor.get_multi(db, skip=skip, limit=limit)
    return doctors


@router.post("/", response_model=Doctor, status_code=status.HTTP_201_CREATED)
def create_doctor(
    *,db: Session = Depends(get_db),
    doctor_in: DoctorCreate,
) ->Any:
    existing_doctor = doctor.get_by_email(db, email = doctor_in.email)
    if existing_doctor:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="The doctor with this email already exists in the system.",
        )
    try: 
        doctor_obj = doctor.create(db, obj_in=doctor_in)
    except IntegrityError:
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Error creating doctor.",
        )
    return doctor_obj

@router.get("/{id}", response_model=Doctor, status_code=status.HTTP_200_OK)
def read_doctor(*, db: Session = Depends(get_db), id: int) -> Any:
    doctor_obj = doctor.get_with_availability(db, id=id)
    if not doctor_obj:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Doctor not found.",
        )
    return doctor_obj


@router.put("/{id}", response_model = Doctor, status_code=status.HTTP_200_OK)
def update_doctor(
    *, db: Session = Depends(get_db),
    id: int, doctor_in: DoctorUpdate
) -> Any:
    doctor_obj = doctor.get(db, id = id)
    if not doctor_obj:
        raise HTTPException(
            status_code = status.HTTP_404_NOT_FOUND,
            detail = "Doctor not found."
        )

    if doctor_in.email is not None and doctor_in.email != doctor_obj.email:
        existing_doctor = doctor.get_by_email(db, email=doctor_in.email)
        if existing_doctor:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="The doctor with this email already exists in the system.",
            )

    try:
        doctor_obj = doctor.update(db, db_obj = doctor_obj, obj_in = doctor_in)
    except IntegrityError:
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Error updating doctor.",
        )
    return doctor_obj


@router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_doctor(*, db: Session = Depends(get_db), id: int) -> None:
    doctor_obj = doctor.get(db, id=id)
    if not doctor_obj:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Doctor not found.",
        )

    try:
        doctor.remove(db, id=id)
    except IntegrityError:
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Error deleting doctor.",
        )

@router.post("/{id}/availability", response_model=DoctorWithAvailability, status_code=status.HTTP_201_CREATED)
def add_doctor_availability(
    *, db: Session = Depends(get_db),
    id: int, availability_in: AvailabilityCreate
) -> Any:
    doctor_obj = doctor.get(db, id=id)
    if not doctor_obj:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Doctor not found.",
        )
    try:
        doctor_obj = doctor.add_availability(db, id = id, availability_in=availability_in)
    except IntegrityError:
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid availability data or time conflict.s",
        )
    return doctor_obj

@router.get("/specialization/{specialization}", response_model=List[Doctor], status_code=status.HTTP_200_OK)
def read_doctors_by_specialization(
    *, db: Session = Depends(get_db), specialization: str
) -> Any:
    doctors = doctor.get_by_specialization(db, specialization=specialization)
    if not doctors:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No doctors found with the specified specialization.",
        )
    return doctors